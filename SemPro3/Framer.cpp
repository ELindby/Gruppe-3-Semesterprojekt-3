...
std::bitset<8> Framer::AddHeader(bool lastPackage, int size) {
	std::string header;
...
void Framer::MessageCutter(std::vector<std::bitset<8>> message) {
...
std::vector<std::vector<std::bitset<8>>> Framer::GetPackages() {
	return slicedMessage;
}

void Framer::AddTrailer(std::vector<std::bitset<8>> headerAndDatagram) {
	CRC crcClass;
